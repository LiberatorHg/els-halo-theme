...
function triggerPosterShare(config) {
  let title_new = config.title.replace(/ - 艾里奥斯拜德中心图书馆/g, '')
  let des = 'By-艾里奥斯拜德中心图书馆 - 仅供参考'
  QRCode.toDataURL(config.url)
    .then(data => {
      ...
      }${config.title !== '' ? `<div class="dshare-poster-content"><p class="dshare-poster-title">${title_new}</p>` : ''
      }<p class="dshare-poster-desc">${des}</p><div class="dshare-poster-footer"><img class="dshare-poster-qrcode" src="${data}" alt="${config.title
